...
const {
  createMedicationTokenSignature,
...
const SEARCH_RESULT_CACHE_LIMIT = 256;
...
const searchResultPromises = new Map();
...
function buildOpenFdaMedicationOptions(records, featuredMedicationIds) {
  ...
  const options = Array.from(groups.values())
    .map((group) => {
      ...
      const queryLabelStrengthAliases = strengthOptions.map((strength) =>
        buildMedicationQueryLabel(
          {
      ...
      const dosageStrengthAliases = strengthOptions
        .map((strength) =>
          group.primaryDosageForm
            ? `${group.optionLabel} ${strength.value} ${group.primaryDosageForm}`
            : null,
        )
        .filter(Boolean);
      const aliases = uniqueStrings([
        ...
        ...dosageStrengthAliases,
        ...queryLabelStrengthAliases,
      ]);
...
function buildSearchResultCacheKey(query, { limit, exact, assetBaseUrl }) {
  return JSON.stringify({
    query: normalizeMedicationText(query),
    exact: Boolean(exact),
    limit: Number(limit),
    assetBaseUrl: getCacheKey(assetBaseUrl),
  });
}

function readSearchResultCache(cacheKey) {
  ...
  if (Date.now() - cached.storedAt > SEARCH_RESULT_CACHE_TTL_MS) {
...
function writeSearchResultCache(cacheKey, value) {
  searchResultCache.delete(cacheKey);
...
function scoreAlias(alias, normalizedQuery, querySignature) {
  if (!normalizedQuery) {
...
function scoreOption(option, normalizedQuery, normalizedIdentifierQuery, queryTokens, exactOnly, querySignature) {
  let bestScore = Number.POSITIVE_INFINITY;

  option.normalizedAliases.forEach((alias) => {
    bestScore = Math.min(bestScore, scoreAlias(alias, normalizedQuery, querySignature));
  });
...
function searchMedicationOptions(
  ...
  const cacheKey = buildSearchResultCacheKey(query, {
    limit,
    exact,
    assetBaseUrl,
  });
  ...
  if (!searchResultPromises.has(cacheKey)) {
    const nextPromise = getMedicationSearchManifest({ assetBaseUrl })
      .then(async (manifest) => {
        const normalizedQuery = normalizeMedicationText(query);
      ...
      .finally(() => {
        searchResultPromises.delete(cacheKey);
      });

    searchResultPromises.set(cacheKey, nextPromise);
  }

  return searchResultPromises.get(cacheKey);
}
...
function resolveMedicationOption(query, { assetBaseUrl } = {}) {
  return searchMedicationOptions(query, {
    ...
    const exactOption = result.results[0] || null;
    ...
    return searchMedicationOptions(query, {
      ...
      fallbackResult.results.length === 1 ? fallbackResult.results[0] : null,
    );
  });
}
