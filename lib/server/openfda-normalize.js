...
function mergeNdcRecord(group, record, referenceDate) {
  ...
  addIngredientAliases(group, ingredients);
...
function addIngredientAliases(group, ingredients) {
  uniqueStrings(
    (Array.isArray(ingredients) ? ingredients : []).map((ingredient) =>
      titleCase(sanitizeText(ingredient?.name)),
    ),
  ).forEach((ingredientAlias) => {
    group.genericNames.add(ingredientAlias);
  });
}
...
function buildSignalAssessment(candidate, shortages, recalls) {
  ...
  if (activeShortages.length) {
    score += 58;
    drivers.push(`${activeShortages.length} current shortage entr${activeShortages.length === 1 ? "y is" : "ies are"} active for a matching presentation.`);
  } else if (availableShortages.length) {
    score += 22;
    drivers.push("A matching product appears in current shortage records, but the listed status is currently available.");
  }
  ...
  if (manufacturerCount <= 1) {
    score += 16;
    drivers.push("Only one manufacturer surfaced in the matching records.");
  } else if (manufacturerCount === 2) {
    score += 8;
    drivers.push("Only two manufacturers surfaced in the matching records.");
  }

  if (candidate.activeListingCount <= 2) {
    score += 10;
    drivers.push("Only a small number of active reference listings matched this search.");
  ...
  const patientSummary =
    level === HIGHER_FRICTION
      ? "Current records suggest this exact medication family may be harder than average to obtain."
      : level === MIXED
        ? "Current records are mixed: this may be obtainable, but there are still supply friction clues worth checking."
        : "Current records do not show a strong supply warning for the matching medication family, but that is still not live store inventory.";
  ...
  const prescriberSummary =
    level === HIGHER_FRICTION
      ...
      : level === MIXED
        ...
        : "The reference data looks relatively stable compared with typical shortage patterns, though retail access can still vary locally.";
  ...
  return {
    ...
    drivers: drivers.length
      ? drivers
      : ["No strong shortage or recall signal surfaced in the matching records."],
    patientSummary,
    prescriberSummary,
  };
}
...
function buildPrescriberTakeaways(candidate, shortages, recalls, signal) {
  ...
  if (candidate.manufacturers.length <= 1) {
    takeaways.push("Manufacturer diversity appears limited in the matching records.");
  } else {
    takeaways.push(`${candidate.manufacturers.length} manufacturers surfaced across the matched records.`);
  }
  ...
  if (candidate.strengths.length <= 2) {
    ...
    takeaways.push(`${candidate.strengths.length} distinct strengths surfaced in the matched records.`);
  }
...
function buildCandidateOutput(candidate, shortagePayload, recallPayload, referenceDate, query) {
  ...
  const knownFacts = [
    activeShortages.length
      ? `${activeShortages.length} active shortage entr${activeShortages.length === 1 ? "y" : "ies"} matched this product family.`
      : "No active shortage entry surfaced for the top matching presentations.",
    `${candidate.activeListingCount} active reference listing${candidate.activeListingCount === 1 ? "" : "s"} across ${candidate.manufacturers.length} manufacturer${candidate.manufacturers.length === 1 ? "" : "s"}.`,
    recentRecalls.length
      ? `${recentRecalls.length} recent recall notice${recentRecalls.length === 1 ? "" : "s"} matched this product family.`
      : "No recent recall notice surfaced for this product family in current records.",
  ];
...
function buildDrugIntelligencePayload({
  ...
  return {
    ...
    matches,
    limitations: [
      "Current records do not provide live retail shelf inventory.",
      "The access signal is inferred from medication listings, shortage records, discontinuation signals, and recall activity.",
      ...UNAVAILABLE_DATA.map((item) => `${item} is not included in these records.`),
    ],
    methodology_summary:
      "PharmaPath reads public medication listing, shortage, approval, and recall records, then translates them into a signal-based access summary without claiming real-time pharmacy stock.",
  };
}
