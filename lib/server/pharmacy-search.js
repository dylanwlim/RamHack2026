...
const MAX_MEDICATION_INPUT_LENGTH = 120;
const MAX_LOCATION_INPUT_LENGTH = 200;
...
const REAL_RESULTS_LABEL =
  "Pharmacy names, addresses, ratings, hours, and map links come from a live nearby search.";
...
function isLocationAddressType(types) {
...
function isLocationAreaType(types) {
  return Array.isArray(types)
    ? types.some((type) =>
        [
          "postal_code",
...
function buildLocationSuggestionFromPrediction(prediction, fallbackDescription) {
  const structuredFormatting = prediction.structured_formatting || {};
  const description =
    sanitizeText(prediction.description) ||
    sanitizeText(structuredFormatting.main_text) ||
    fallbackDescription;
...
function buildLocationSuggestionFromResolvedLocation(location) {
  const secondaryText = [
    location.city,
    location.state,
    location.country,
  ...
  const types = Array.from(
    new Set(["postal_code", ...(Array.isArray(location.types) ? location.types : [])]),
  );

  return {
    place_id: sanitizeText(location.place_id) || null,
    description: sanitizeText(location.display_label || location.formatted_address),
    primary_text: sanitizeText(location.postal_code || location.display_label || location.formatted_address),
    secondary_text: sanitizeText(secondaryText || location.formatted_address) || null,
    types,
    type_label: "ZIP",
  };
}

function isExactPostalSuggestion(suggestion, normalizedQuery) {
  const normalizedPrimary = normalizeLocationSearchText(suggestion.primary_text);
  const normalizedDescription = normalizeLocationSearchText(suggestion.description);

  return (
    Array.isArray(suggestion.types) &&
    suggestion.types.includes("postal_code") &&
    (normalizedPrimary === normalizedQuery ||
      normalizedDescription.startsWith(`${normalizedQuery} `) ||
      normalizedDescription.includes(` ${normalizedQuery}`))
  );
}

function scoreLocationSuggestion(suggestion, normalizedQuery, exactPostalQuery) {
  const normalizedPrimary = normalizeLocationSearchText(suggestion.primary_text);
  const normalizedDescription = normalizeLocationSearchText(suggestion.description);
  const normalizedSecondary = normalizeLocationSearchText(suggestion.secondary_text);
  let score = 0;

  if (exactPostalQuery) {
    if (isExactPostalSuggestion(suggestion, normalizedQuery)) {
      score += 1000;
    }
    ...
    if (isLocationAreaType(suggestion.types)) {
      score += 180;
    }
    ...
    if (isLocationAddressType(suggestion.types) && normalizedPrimary.startsWith(normalizedQuery)) {
      score -= 520;
    }
  } else {
    if (normalizedPrimary === normalizedQuery) {
...
function rankLocationSuggestions(suggestions, query, limit) {
  const normalizedQuery = normalizeLocationSearchText(query);
  const exactPostalQuery = isLikelyUsPostalCode(query);
  const deduped = [];
  ...
  for (const suggestion of suggestions) {
    ...
    const dedupeKey = `${sanitizeText(suggestion.place_id)}::${normalizeLocationSearchText(suggestion.description)}`;
  ...
  const hasExactPostalSuggestion = exactPostalQuery
    ? deduped.some((suggestion) => isExactPostalSuggestion(suggestion, normalizedQuery))
    : false;
  const filtered = hasExactPostalSuggestion
    ? deduped.filter((suggestion) => {
        const normalizedPrimary = normalizeLocationSearchText(suggestion.primary_text);
  ...
  return filtered
    .sort((left, right) => {
      const scoreDelta =
        scoreLocationSuggestion(right, normalizedQuery, exactPostalQuery) -
        scoreLocationSuggestion(left, normalizedQuery, exactPostalQuery);
...
function buildPlaceWorkflow(place, medicationProfile, medication) {
  ...
  if (medicationProfile.key === "controlled_stimulant") {
    ...
    return {
      ...
      inventory_note: `${place.name} is a real nearby result, but ${medication} stock is not live-verified in PharmaPath.`,
    };
  }
  ...
  if (medicationProfile.key === "acute_antibiotic") {
    ...
    return {
      ...
      inventory_note: `${place.name} is a real nearby result, but PharmaPath is not confirming ${medication} inventory live.`,
    };
  }
  ...
  if (medicationProfile.key === "cold_chain") {
    ...
    return {
      ...
      inventory_note: `${place.name} is a real nearby result, but PharmaPath does not have live cold-chain inventory data.`,
    };
  }
  ...
  return {
    ...
    inventory_note: `${place.name} is a real nearby result, but ${medication} stock still needs direct confirmation.`,
  };
}
...
async function fetchJson(url) {
  return googleRequestCache.getOrLoad(url.toString(), async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw createGoogleApiError(
          `Location service request failed with HTTP ${response.status}.`,
          response.status,
      ...
      if (status === "OVER_QUERY_LIMIT" || status === "REQUEST_DENIED") {
        throw createGoogleApiError(
          payload?.error_message || `Location service failed with status ${status}.`,
          503,
          status.toLowerCase(),
        );
      }
      ...
      if (error.name === "AbortError") {
        throw createGoogleApiError("Location service request timed out.", 504, "google_api_timeout");
      }
...
async function autocompleteLocationSuggestions(query, apiKey, { limit = 8, sessionToken } = {}) {
  const normalizedQuery = sanitizeText(query).slice(0, MAX_LOCATION_INPUT_LENGTH);
  ...
  const exactPostalSuggestionPromise = isLikelyUsPostalCode(normalizedQuery)
    ? geocodeLocation(normalizedQuery, apiKey, {
        ...
        .then((location) => buildLocationSuggestionFromResolvedLocation(location))
        .catch(() => null)
    : Promise.resolve(null);
  const [payload, exactPostalSuggestion] = await Promise.all([
    fetchJson(url),
    exactPostalSuggestionPromise,
  ]);

  if (payload.status === "ZERO_RESULTS") {
    return exactPostalSuggestion ? [exactPostalSuggestion] : [];
  }
  ...
  const suggestions = payload.predictions.map((prediction) =>
    buildLocationSuggestionFromPrediction(prediction, normalizedQuery),
  );

  return rankLocationSuggestions(
    exactPostalSuggestion ? [exactPostalSuggestion, ...suggestions] : suggestions,
    normalizedQuery,
    limit,
  );
}
...
async function hydratePlacePhoneDetails(place, apiKey) {
  ...
  try {
    return await placePhoneDetailsLimiter.run(async () => ({
      ...place,
...
async function resolveLocationInput({ query, placeId, sessionToken } = {}, apiKey) {
  const normalizedQuery = sanitizeText(query).slice(0, MAX_LOCATION_INPUT_LENGTH);
  const normalizedPlaceId = sanitizeText(placeId).slice(0, MAX_PLACE_ID_LENGTH);
...
function getSearchInput(req, body = {}) {
  ...
  return {
    medication: sanitizeText(source.medication || source.query).slice(0, MAX_MEDICATION_INPUT_LENGTH),
    location: sanitizeText(source.location).slice(0, MAX_LOCATION_INPUT_LENGTH),
    locationPlaceId: sanitizeText(source.locationPlaceId || source.location_place_id).slice(0, MAX_PLACE_ID_LENGTH),
